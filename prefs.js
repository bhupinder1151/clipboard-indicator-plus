...
class Settings {
    constructor(schema, window) {
        ...
        // Custom storage location settings
        this.field_use_custom_storage = new Adw.SwitchRow({
            title: _("Use custom storage location"),
            subtitle: _("Store clipboard history in a custom folder instead of ~/.cache/")
        });

        this.field_custom_storage_path = new Adw.ActionRow({
            title: _("Storage folder"),
            subtitle: this.schema.get_string(PrefsFields.CUSTOM_STORAGE_PATH) || _("Not set")
        });
        ...
        this.field_custom_storage_path.add_suffix(browseButton);
        this.field_custom_storage_path.set_activatable_widget(browseButton);

        // Update sensitivity based on toggle state
        this.field_use_custom_storage.connect('notify::active', (widget) => {
            this.field_custom_storage_path.set_sensitive(widget.active);
        });
        ...
        this.storage = new Adw.PreferencesGroup({
            title: _('Storage'),
            description: _('Changes require restarting GNOME Shell to take effect')
        });
        ...
        this.storage.add(this.field_use_custom_storage);
        this.storage.add(this.field_custom_storage_path);
        ...
        this.schema.bind(PrefsFields.USE_CUSTOM_STORAGE, this.field_use_custom_storage, 'active', Gio.SettingsBindFlags.DEFAULT);

        this.field_clear_history_interval.set_sensitive(this.field_clear_history_on_interval.active);
        this.field_custom_storage_path.set_sensitive(this.field_use_custom_storage.active);
        this.#fetchExludedAppsList();
    }

    #openFolderChooser() {
        const dialog = new Gtk.FileDialog({
            title: _("Select Storage Folder"),
            modal: true
        });

        dialog.select_folder(this.window, null, (dialog, result) => {
            try {
                const folder = dialog.select_folder_finish(result);
                if (folder) {
                    const path = folder.get_path();
                    this.schema.set_string(PrefsFields.CUSTOM_STORAGE_PATH, path);
                    this.field_custom_storage_path.set_subtitle(path);
                }
            } catch (e) {
