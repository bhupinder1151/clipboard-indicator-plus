...
import { DialogManager, QRDialogManager } from './confirmDialog.js';
import { PrefsFields } from './constants.js';
import { Keyboard } from './keyboard.js';
import { generateQRCode, TextServer } from './qrcode.js';
...
const ClipboardIndicator = GObject.registerClass({
    ...
    _init(extension) {
        ...
        this.qrDialogManager = new QRDialogManager();
    ...
    _addEntry(entry, autoSelect, autoSetClip) {
        ...
        // QR Code button (only for text entries)
        if (entry.isText()) {
            menuItem.qrBtn = new St.Button({
                style_class: 'ci-action-btn',
                can_focus: true,
                child: new St.Icon({
                    icon_name: 'emblem-shared-symbolic',
            ...
            menuItem.qrBtn.connect('clicked', () => {
                this.#generateAndShowQR(menuItem);
            });

            menuItem.actor.add_child(menuItem.qrBtn);
        }
    ...
    async #generateAndShowQR(menuItem) {
        const text = menuItem.entry.getStringValue();
        if (!text) return;
