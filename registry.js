...
export class Registry {
    constructor({ settings, uuid }) {
        ...
        // Check if custom storage is enabled and has a valid path
        const useCustomStorage = settings.get_boolean(PrefsFields.USE_CUSTOM_STORAGE);
        const customPath = settings.get_string(PrefsFields.CUSTOM_STORAGE_PATH);

        if (useCustomStorage && customPath && customPath.trim() !== '') {
            this.REGISTRY_DIR = customPath;
        } else {
